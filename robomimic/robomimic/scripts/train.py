...
def train(config, device):
    ...
    if env_meta["env_kwargs"]["camera_heights"] != 96:
    ...
    data_logger = DataLogger(
        ...
        log_neptune=config.experiment.logging.log_neptune,
    )
    model = algo_factory(
    ...
    for epoch in range(1, config.train.num_epochs + 1): # epoch numbers start at 1
        ...
        if video_paths is not None and should_save_video:
            try:
                for env_name in video_paths:
                    data_logger.record(
                        k="Video/{}".format(env_name),
                        v=video_paths[env_name],
                        epoch=epoch,
                        data_type="video"
                    )
            except Exception as e:
                print("Failed to save video paths to Neptune: {}".format(e))
...
def main(args):
    ...
    if args.config is not None:
        ...
        # Here I unlock the keys also to allow for adding the camera randomization params
